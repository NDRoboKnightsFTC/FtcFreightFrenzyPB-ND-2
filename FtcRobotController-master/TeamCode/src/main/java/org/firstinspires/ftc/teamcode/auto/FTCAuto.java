    ...
    private void doCommand(RobotXMLElement pAction) throws XPathException {
        ...
        switch (commandName) {
            ...
            case "FORWARD_UNTIL_DISTANCE_OR_TOUCH": {
                ...
                double velocity = Math.abs(commandXPath.getDouble("velocity")); //fraction of max
                if (velocity <= 0.0 || velocity > 1.0)
                    throw new AutonomousRobotException(TAG, "velocity out of range " + velocity);

                //!! FTS convention: you must set target clicks before RUN_TO_POSITION.
                robot.leftFrontMotor.setTargetPosition(targetClicks);
                robot.leftFrontMotor.setMode(DcMotor.RunMode.RUN_TO_POSITION);

                try {
                    robot.leftFrontMotor.setPower(velocity); // robot moves
                    while (linearOpMode.opModeIsActive() &&
                            robot.leftFrontMotor.isBusy() && !robot.digitalTouch.isPressed()) {
