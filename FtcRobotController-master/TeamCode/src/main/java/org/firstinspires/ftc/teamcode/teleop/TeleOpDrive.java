...
public class TeleOpDrive extends LinearOpMode {
    ...
    private void checkBothBumpersNotHeld(){
        if ((!leftBumper.is(FTCButton.State.HELD) && !rightBumper.is(FTCButton.State.HELD)) ||
                leftBumper.is(FTCButton.State.HELD) && rightBumper.is(FTCButton.State.HELD)){
            robot.leftFrontMotor.setPower(0.0);
            telemetry.addData("Stop", "Stop Motor");
            telemetry.update();
        }
    }
